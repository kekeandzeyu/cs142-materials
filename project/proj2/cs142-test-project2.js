...
(function () {  // Start of IIFE (Immediately Invoked Function Expression)
    ...
    const letDeclared = ["letDeclared", "p1Message", "p2Message", "p3Message"];
            ...
            result = filterFuncTwo(
                ...
                function (callbackResult) {
                    if (!arraysAreTheSame([2, 3, 4], callbackResult)) {
                        console.error(
                            "second filter does not filter 1 (check for global letiable usage)",
                            callbackResult,
                        );
                        p1Message = "FAILURE";
                    }
    ...
    if (typeof Cs142TemplateProcessor !== "function") {
        ...
        letDeclared.push("template");
        letDeclared.push("dateTemplate");
        letDeclared.push("dictionary");
        letDeclared.push("str");
    }
    console.log("Test Cs142TemplateProcessor:", p2Message);

    // *** Test to see if the symbols we defined are in the global address space ***

    letDeclared.forEach(function (sym) {
